...
import indi.df.fmall.util.CookieUtil;
    ...
    public String addToCart(String skuId, int quantity, HttpServletRequest request, HttpServletResponse response, HttpSession session){
        ...
        if(StringUtils.isBlank(memberId)){
            ...
            if (StringUtils.isBlank(cartListCookie)) {
                ...
                // cookie不为空
                omsCartItems = JSON.parseArray(cartListCookie, OmsCartItem.class); //将从redis中获得的字符串直接转换成对象
                // 判断新添加的购物车数据在之前保存的cookie中是否存在
            ...
            if(omsCartItemFromDb==null){
                // 该用户没有添加过当前商品
                omsCartItem.setMemberId(memberId); //因为确认当前用户已经登陆，所以才可以将memberId添加进去，之前不行
