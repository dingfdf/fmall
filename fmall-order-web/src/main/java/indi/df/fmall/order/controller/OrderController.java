...
import indi.df.fmall.annotations.LoginRequired;
import indi.df.fmall.bean.OmsCartItem;
import indi.df.fmall.bean.OmsOrder;
import indi.df.fmall.bean.OmsOrderItem;
...
public class OrderController {
    ...
    public String toTrade(HttpServletRequest request, HttpServletResponse response, HttpSession session, ModelMap modelMap){
        ...
        // 将购物车集合转化为页面计算清单集合
        List<OmsCartItem> omsCartItems = cartService.cartList(memberId); //获得购物车列表
        List<OmsOrderItem> omsOrderItems = new ArrayList<>(); //创建订单列表

        // 每循环一个购物车对象，就封装一个商品的详情到OmsOrderItem, 但是没有存储进数据库
        for(OmsCartItem omsCartItem : omsCartItems){
            //如果购物车对象被勾选，就将其封装进订单对象
        ...
        // 生成交易码，为了在提交订单时做交易码的校验，防止反复提交订单
    ...
    private BigDecimal getTotalAmount(List<OmsCartItem> omsCartItems) {
        ...
        for (OmsCartItem omsCartItem : omsCartItems) {
            ...
            if (true/*omsCartItem.getIsChecked().equals("1")*/) {
    ...
    public ModelAndView submitOrder(String receiveAddressId, BigDecimal totalAmount, String tradeCode, HttpServletRequest request, HttpServletResponse response, HttpSession session, ModelMap modelMap) {
        ...
        if(success.equals("success")){
            ...
            // 重定向到支付系统
            ModelAndView mv = new ModelAndView("redirect:http://payment.fmall.com:8087/index");
